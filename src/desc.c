...
void magic_init(__attribute__((unused)) unsigned long random_seed)
{
	...
	for (i1 = 1; i1 <= MAX_OBJECTS; i1++) {
		...
		case potion2:
			if (tmpv <= MAX_COLORS) {
				insert_str(object_list[i1].name, "%C",
					   colors[tmpv - 1]);
			}
			break;
		...
		case ring:
			if (tmpv <= MAX_ROCKS) {
				insert_str(object_list[i1].name, "%R",
					   rocks[tmpv - 1]);
			}
			break;
		...
		case amulet:
			if (tmpv <= MAX_AMULETS) {
				insert_str(object_list[i1].name, "%A",
					   amulets[tmpv - 1]);
			}
			break;
		...
		case wand:
			if (tmpv <= MAX_METALS) {
				insert_str(object_list[i1].name, "%M",
					   metals[tmpv - 1]);
			}
			break;
		...
		case horn:
			if (tmpv <= MAX_HORNS) {
				insert_str(object_list[i1].name, "%H",
					   horns[tmpv - 1]);
			}
			break;
		...
		case staff:
			if (tmpv <= MAX_WOODS) {
				insert_str(object_list[i1].name, "%W",
					   woods[tmpv - 1]);
			}
			break;
		...
		case bag_or_sack:
			if (tmpv <= MAX_CLOTHS) {
				insert_str(object_list[i1].name, "%N",
					   cloths[tmpv - 1]);
			}
			break;
...
void identify(treasure_type *item)
{
	...
	if (strstr(item->name, "|") != NULL) {
		...
		for (i1 = Equipment_min; i1 <= Equipment_secondary; i1++) {
			...
			if ((equipment[i1].tval == x1) &&
			    (equipment[i1].subval == x2)) {
		...
		do {
			...
			if ((object_list[i1].tval == x1) &&
				...
				if (strstr(object_list[i1].name, "%T") !=
				    NULL) {
...
void objdes(char *out_val, treas_ptr ptr, boolean pref)
{
	...
	if (pref) {
		if (strstr(tmp_val, "&") != NULL) {
			...
			if (ptr->data.number > 1) {
				sprintf(out_val, "%d%s", ptr->data.number,
					tmp_val);
