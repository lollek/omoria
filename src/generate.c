		...
		coords *doorstk)
{
	...
	do {
		if (randint(100) > DUN_TUN_CHG) {
			gc__rand_dir(&row_dir, &col_dir, row1, col1, row2, col2,
				     DUN_TUN_RND);
		}
	...
	for (long i = 1; i <= wallptr; i++) {
		if (cave[wallstk[i].y][wallstk[i].x].fval == 9) {
			if (randint(100) < DUN_TUN_PEN) {
				...
				cave[wallstk[i].y][wallstk[i].x].fval =
				    corr_floor2.ftval;
				cave[wallstk[i].y][wallstk[i].x].fopen =
				    corr_floor2.ftopen;
			}
		}
	}
...
static void gc__cave_gen()
{
	...
	if (1) {
		for (long i = 1, roomCount = randnor(DUN_ROO_MEA, 2);
		     i <= roomCount; i++) {
		...
		for (long i2 = 1; i2 <= col_rooms; i2++) {
			if (room_map[y][i2]) {
				...
				if (dun_level > randint(dun_unusual)) {
					switch (randint(3)) {
					case 1:
						gc__build_type1(yloc[i3],
								xloc[i3]);
						break;
					case 2:
						gc__build_type2(yloc[i3],
								xloc[i3]);
						break;
					case 3:
						gc__build_type3(yloc[i3],
								xloc[i3]);
						break;
					}
				} else {
	...
	alloc_land_monster(allocSet2,
			   (randint(8) + MIN_MALLOC_LEVEL + alloc_level) / 3, 0,
			   true, true);
...
static void gc__build_store(enum store_t store_num, long where)
{
	...
	if (house_type == 3) {
		gc__make_door(y_height, xval - 2 + randint(4), &cur_pos,
			      store_num, house_type);
		gc__make_door(y_depth, xval - 2 + randint(4), &cur_pos,
			      store_num, house_type);
		gc__make_door(yval - 2 + randint(3), x_left, &cur_pos,
			      store_num, house_type);
		gc__make_door(yval - 2 + randint(3), x_right, &cur_pos,
			      store_num, house_type);
	} else if (house_type == 4) {
		gc__make_door(yval - 1, xval - 4, &cur_pos, store_num,
			      house_type);
		gc__make_door(yval + 1, xval - 4, &cur_pos, store_num,
			      house_type);
		gc__make_door(yval - 1, xval + 4, &cur_pos, store_num,
		...
		i1 = 2 * randint(2) - 3;
		gc__make_door(yval + 2 * i1, xval - 1, &cur_pos, store_num,
			      house_type);
		gc__make_door(yval + 2 * i1, xval, &cur_pos, store_num,
			      house_type);
		gc__make_door(yval + 2 * i1, xval + 1, &cur_pos, store_num,
			      house_type);
		...
		if (store_num == S_BANK) {
			gc__make_door(q1, q2, &cur_pos, S_INSURANCE,
				      house_type);
		}
	}
}
