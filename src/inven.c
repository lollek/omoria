	...
	for (count = 0; (start != NULL) && (count < DISPLAY_SIZE);) {
		if (start->ok) {
			...
			if (cur_display[count] != start) {
				...
				if (start->is_in) {
					sprintf(out_val2, "%c%c%c%s%s",
						cur_insure(), (96 + (int)count),
						cur_char1(), "     ", out_val);
				} else {
					sprintf(out_val2, "%c%c%c%s%s",
						cur_insure(), (96 + (int)count),
	       ...
	       integer *scr_state, boolean *valid_flag)
{
	...
	do {
		...
		ic__clear_display(cur_display, cur_display_size);
		exit_flag = !(ic__show_inven(&item_ptr, true, false, scr_state,
					     valid_flag, prompt, cur_display,
					     cur_display_size));
		if (item_ptr != NULL) {
			test_flag = true;
			...
			case song_book:
				...
				break;
			default:
				sprintf(out_val, "Unknown item type: %d",
					item_ptr->data.tval);
				break;
			}
			...
			prt(out_val, 8, 15);
			sprintf(out_val, "%ld iron  (%ld gold)",
				item_ptr->data.cost,
			...
			if (get_item(&into_ptr, "Into which item?", &redraw,
				...
				if (into_ptr == put_ptr) {
					msg_print("You can't seem to fit it "
						  "inside itself.");
				} else if ((put_ptr->data.flags2 &
					    Holding_bit) != 0) {
					msg_print("Uh oh, now you've done it!");
					msg_print(
					    "You lose the items in both bags!");
					...
					inven_weight -= (put_ptr->data.weight *
							 put_ptr->data.number);
					msg_print("You stuff it inside");

					if ((put_ptr->data.flags2 &
					     Sharp_bit) != 0) {
			 ...
			 treas_ptr cur_display[], integer *cur_display_size)
{
	...
	*(--out_pos) = 0;
	...
	do {
		sprintf(out_str, "What type of items to inventory? (%s) ",
			out_pos);
		prt(out_str, 1, 1);
		if (!(get_com("", &command))) {
			exit_flag = true;
	...
	if (!exit_flag) {
		...
		clear_rc(1, 1);
		strcpy(prompt,
		       "You are currently carrying: space for next page");
		ic__show_inven(&ptr, false, false, scr_state, valid_flag,
	...
	do {
		...
		case 'i': /*{ Inventory     }*/
			if (inven_ctr == 0) {
				...
				ic__clear_display(cur_display,
						  &cur_display_size);
				change_all_ok_stats(true, true);
				ic__show_inven(item_ptr, false, false,
					       &scr_state, &valid_flag, prompt,
		...
		case 'c':
			if (inven_ctr == 0) {
				...
				ic__clear_display(cur_display,
						  &cur_display_size);
				change_all_ok_stats(true, true);
				ic__show_inven(item_ptr, true, true, &scr_state,
					       &valid_flag, prompt, cur_display,
					       &cur_display_size);
			}
		...
		case 's': /*{ Statistics of an item }*/
			...
			if ((!(wizard1) && !(wizard2)) && 0) {
				...
				if (inven_ctr == 0) {
					...
					ic__stats(cur_display,
						  &cur_display_size, prompt,
		...
		case 'w': /*{ Wear/wield    }*/
			if (inven_ctr == 0) {
				...
				ic__wear(cur_display, &cur_display_size, prompt,
					 &scr_state, &valid_flag);
			}
			break;
		...
		case 'I':
			if (inven_ctr == 0) {
				...
				ic__selective_inven(&scr_state, &valid_flag,
						    prompt, cur_display,
						    &cur_display_size);
			}
			break;
		...
		case '?': /* { Displays part inven, returns  }*/
			cur_inven = inventory_list;
			ic__clear_display(cur_display, &cur_display_size);
			return_value = ic__show_inven(
			    item_ptr, true, false, &scr_state, &valid_flag,
			    prompt, cur_display, &cur_display_size);
...
void delete_inven_item(treas_ptr ptr)
{
	...
	if (ptr == inventory_list) {
		...
		for (curse = inventory_list; curse->next != ptr;
		     curse = curse->next) {
		}
