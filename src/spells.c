...
/*//////////////////////////////////////////////////////////////////// */
/*//////////////////////////////////////////////////////////////////// */
boolean explode(integer typ, integer y, integer x, integer dam_hp,
		ctype descrip)
{
	integer i1, i2;
	...
	if (thit == 1) {
		...
		sprintf(out_val, "The %s envelopes several creatures!",
			descrip);
		msg_print(out_val);
	}
	...
	if (tkill == 1) {
		if (typ == c_joke) {
			msg_print(
			    "There is a scream of side-splitting laughter!");
		} else {
		...
		if (typ == c_joke) {
			msg_print(
			    "There are several screams of agonized laughter!");
		} else {
...
boolean restore_level()
{
	...
	while ((integer)(player_exp[max_level] * py.misc.expfact) <=
	       py.misc.max_exp) {
		max_level++;
	}
...
boolean detect_creatures(integer typ)
{
	...
	do {
		...
		if (panel_contains(m_list[i1].fy, m_list[i1].fx)) {
			...
			if (found) {
				m_list[i1].ml = true;
				print(c_list[m_list[i1].mptr].cchar,
				      m_list[i1].fy, m_list[i1].fx);
				flag = true;
			}
		}
...
boolean detect_item(integer typ)
{
	...
	integer i1, i2;
	boolean show_it;
	obj_set treasures = {valuable_jewelry, valuable_gems, valuable_metal,
			     0};
	boolean flag = false;
		...
		for (i2 = panel_col_min; i2 <= panel_col_max; i2++) {
			...
			if (cave[i1][i2].tptr > 0) {
				if (t_list[cave[i1][i2].tptr].tval ==
					...
					cave[i1][i2].fm = true;
					flag = true;
				} else if (t_list[cave[i1][i2].tptr].tval ==
					   chest) {
					/* with t_list[tptr] do; */
		...
		for (i2 = panel_col_min; i2 <= panel_col_max; i2++) {
			...
			if (cave[i1][i2].tptr > 0) {
				...
				if (t_list[cave[i1][i2].tptr].tval ==
					...
					/*{ Staircases    }*/
				} else if (is_in(t_list[cave[i1][i2].tptr].tval,
						 pick_a_stair)) {
					if (!(cave[i1][i2].fm)) {
						cave[i1][i2].fm = true;
						lite_spot(i1, i2);
...
boolean ident_spell()
{
	...
	for (ptr = inventory_list; ptr != NULL; ptr = ptr->next) {
		if (strchr(ptr->data.name, '^') ||
		    strchr(ptr->data.name, '|')) {
			ptr->ok = true;
			count++;
		}
	}
	...
	if (count == 0) {
		...
		if (get_item(&item_ptr, "Item you wish identified?", &redraw,
			     count, &trash_char, false, false)) {
	...
	for (i1 = y - 1; i1 <= y + 1; i1++) {
		for (i2 = x - 1; i2 <= x + 1; i2++) {
			...
			if (cave[i1][i2].cptr > 1) {
				...
				/* with c_list[m_list[cave[i1][i2].cptr].mptr].
				 */
				/* do; */
				if (!mon_resists(cave[i1][i2].cptr)) {
					flag = true;
					if (mon_save(cave[i1][i2].cptr, 0,
						     c_sc_mental)) {
						sprintf(
						    out_val,
						    "The %s is unaffected.",
...
boolean td_destroy()
{
	...
	for (i1 = char_row - 1; i1 <= char_row + 1; i1++) {
		for (i2 = char_col - 1; i2 <= char_col + 1; i2++) {
			/* with cave[i1][i2]. do; */
			if (cave[i1][i2].tptr > 0) {
				if (is_in(t_list[cave[i1][i2].tptr].tval,
					  pick_a_door)) {
		...
		if (is_in(item_ptr->data.tval, batteries_not_included)) {
			if (randint(8) == 1) {
				...
				if (strchr(item_ptr->data.name, '^') != NULL) {
					insert_str(item_ptr->data.name, " (%P1",
						   "^ (%P1");
				}
			}
...
boolean create_food(integer t0, integer t1, integer t2, integer t3, integer t4)
{
	...
	for (i1 = char_row - 2; i1 <= char_row + 2; i1++) {
		for (i2 = char_col - 2; i2 <= char_col + 2; i2++) {
			...
			if ((labs(i1 - char_row) == 2) ||
			    (labs(i2 - char_col) == 2)) {
				dist++;
			}

			if ((cave[i1][i2].fopen) && (cave[i1][i2].tptr == 0) &&
			    (dist < 5)) {
			...
			integer typ)
{
	...
	case c_speed:
		hmm = !mon_save(c_list[m_list[monptr].mptr].level, 0, c_null) ||
		      (dmge > 0);
		break;
...
/*//////////////////////////////////////////////////////////////////// */
void za__yes_it_did(integer monptr, integer cflag, integer dmge, integer typ,
		    integer i1)
{
	vtype out_val;
	...
	case c_holy_word:
		...
		if (mon_take_hit(monptr, randint(dmge)) > 0) {
			if (typ == c_joke) {
				sprintf(out_val, "The %s dies laughing!",
					c_list[mptr].name);
			} else {
				sprintf(out_val, "The %s dissolves!",
					c_list[mptr].name);
			}
			msg_print(out_val);
		} else {
			if (typ == c_joke) {
				sprintf(out_val, "The %s chuckles.",
					c_list[mptr].name);
				msg_print(out_val);
				m_list[monptr].confused = true;
			} else {
				sprintf(out_val, "The %s shudders.",
					c_list[mptr].name);
				msg_print(out_val);
				if (typ == c_holy_word) {
					if (do_stun(monptr, -4,
	...
	for (; cave[y][x].fopen;) {
		...
		if (panel_contains(y, x)) {
			...
			if (cave[y][x].cptr > 1) {
				...
				/* with c_list[m_list[cave[y][x].cptr].mptr]. */
				/* do; */
...
boolean fb__ill_joke(integer a_cptr, integer typ, integer dam, char *str,
		     char *str2)
{
	integer i2;
	vtype out_val;
	...
	if (!mon_save(a_cptr, 0, c_sc_mental)) {
		...
		if (typ == c_illusion) {
			sprintf(out_val, "%s seems to believe the illusion...",
				str2);
			msg_print(out_val);
			if (i2 > 0) {
...
/*//////////////////////////////////////////////////////////////////// */
/*//////////////////////////////////////////////////////////////////// */
boolean fire_ball(integer typ, integer dir, integer y, integer x,
...
boolean poly_monster(integer dir, integer y, integer x)
{
	...
	do {
		if (bolt_to_creature(dir, &y, &x, &dist, OBJ_BOLT_RANGE,
				     false)) {
			/* with cave[y][x]. do; */
			cptr = cave[y][x].cptr;
...
boolean clone_monster(integer dir, integer y, integer x)
{
	...
	if (move_to_creature(dir, &y, &x)) {
		/* with cave[y][x]. do; */
		if (!mon_resists(cave[y][x].cptr)) {
			multiply_monster(y, x, m_list[cave[y][x].cptr].mptr,
					 false);
			if (panel_contains(y, x)) {
...
/*//////////////////////////////////////////////////////////////////// */
/*//////////////////////////////////////////////////////////////////// */
boolean zm__did_it_work(integer zaptype, integer cptr, integer aux)
{
	boolean flag;
	...
	case c_confuse:
		flag =
		    ((!mon_save(cptr, 0, c_sc_mental)) && (!mon_resists(cptr)));
		break;
	...
	case c_speed:
		flag = ((!mon_save(cptr, 0, c_null)) && (!mon_resists(cptr))) ||
		       (aux > 0);
		break;
...
/*//////////////////////////////////////////////////////////////////// */
void zm__yes_it_did(integer zaptype, integer cptr, integer aux, char *str1,
		    char *str2)
{
	integer i1;
	vtype out_val;
	...
	case c_probe:
		...
		sprintf(out_val, "This monster has %d hp left.",
			m_list[cptr].hp);
		msg_print(out_val);
		break;
			 ...
			 integer max_dist, boolean visable)
{
	...
	do {
	    ...
	    !((!cave[*y][*x].fopen) || (*dist > max_dist) || hit_creature));
...
boolean detect_magic()
{
	...
	if (get_item(&item_ptr, "Item you wish to examine?", &redraw, inven_ctr,
		...
		if ((((item_ptr->data.flags != 0) ||
		      (item_ptr->data.tohit > 0) ||
		      (item_ptr->data.todam > 0) ||
		      (item_ptr->data.toac > 0)) &&
		     !dumb) ||
		    dumber) {
...
/*//////////////////////////////////////////////////////////////////// */
/*//////////////////////////////////////////////////////////////////// */
boolean fire_line(integer typ, integer dir, integer y, integer x,
		  integer dam_hp, ctype descrip)
{
