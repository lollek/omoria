...
static void bj__display_rules() {
  ...
  prt("value.", 8, 2);
  prt("Once you have played your hand, the dealer will draw cards until", 10,
      2);
  ...
  prt("If your first two cards are the same, you may split, in which "
      "case",
      19, 2);
  prt("you will then play out both hands, each with the same bet as the", 20,
      2);
...
static void bj__get_winning() {
  ...
  if (deal_bust) {
    ...
    if (vald > valp) {
      switch (randint(5)) {
      case 4:
        msg_print("Now you know why gambling is illegal.");
        break;
      case 5:
        msg_print("If you had just taken one more card...");
        break;
      default:
        msg_print("You lose.");
        break;
