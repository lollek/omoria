...
static void bj__display_bj()
{
	...
	c__display_gold();
	prt("You may:", 22, 2);
	prt(" p) place a bet.                         v) view the rules", 23,
	    2);
	prt("^R) Redraw the screen.                 Esc) Exit from building.",
	    24, 2);
}
...
static void bj__display_bj_game()
{
	...
	prt("^R) Redraw the screen.                 v) view the rules. ", 24,
	    2);
}
...
static void bj__display_rules()
{
	...
	prt("The object, of course, is to get as close as you can to 21 "
	    "without",
	    5, 2);
	prt("going over.  Aces can count as either one or eleven, while all", 6,
	    2);
	prt("face cards count as ten.  The other cards are worth their face", 7,
	    2);
	prt("value.", 8, 2);
	prt("Once you have played your hand, the dealer will draw cards until",
	    10, 2);
	prt("he has at least 17.  However, if his total is 17 and he has an",
	    11, 2);
	prt("ace, he will hit. ", 12, 2);
	prt("Splitting and Doubling:", 14, 25);
	prt("After drawing your first two cards, you have the option of", 16,
	    2);
	prt("doubling your bet and taking one, and only one more card. ", 17,
	    2);
	prt("If your first two cards are the same, you may split, in which "
	    "case",
	    19, 2);
	prt("you will then play out both hands, each with the same bet as the",
	    20, 2);
	...
	prt("A natural pays  3/2  times your bet, unless the dealer also has",
	    3, 2);
	prt("a blackjack, in which case you push.  The dealer will not offer ",
	    4, 2);
	prt("you insurance.", 5, 2);
	prt("If you draw five cards, without going over 21, you automatically",
	    7, 2);
...
static void bj__get_winning()
{
	...
	if (deal_bust) {
		...
		switch (randint(4)) {
		case 4:
			msg_print("The dealer busts.");
			break;
		default:
			msg_print("The dealer busts, you win.");
			break;
		}
	} else {
		if (vald > valp) {
			...
			case 4:
				msg_print(
				    "Now you know why gambling is illegal.");
				break;
			case 5:
				msg_print(
				    "If you had just taken one more card...");
				break;
			default:
				msg_print("You lose.");
				break;
			...
			switch (randint(4)) {
			case 4:
				msg_print("You're not card-counting are you?");
				break;
			default:
				msg_print("You win.");
				break;
		...
		if (!(card5 || bust_flag || blackjack)) {
			...
			if (bust_flag) {
				...
				case 3:
					...
					break;
				case 2:
					msg_print("You should have stood.");
					break;
				}
			}
			if (blackjack) {
				...
				bj__card_draw(1, 13, dummyd[1]);
				if (vald == 21) {
					msg_print("The dealer also has 21 ... "
						  "You push.");
					gld += bet;
				} else {
					gld += 2 * bet + bet div 2;
