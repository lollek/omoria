...
static void mt__ego_sword(treasure_type *treasure_ptr)
{
	...
	case 4: /* {Soul Sword}*/
		...
		treasure_ptr->flags2 |=
		    (Soul_Sword_worn_bit | Bad_repute_worn_bit);
...
static void mt__gems(treasure_type *treasure_ptr)
{
	...
	case 11:
		...
		break;
	default:
		MSG(("WARNING: Unknown subval in mt__gems: %ld  %s",
		     treasure_ptr->subval, treasure_ptr->name));
		p1 = 0;
		break;
	}
...
static void mt__misc_usable(treasure_type *treasure_ptr)
{
	...
	case 18:
		...
		case 4:
			...
			treasure_ptr->p1 =
			    randint(trunc(treasure_ptr->subval / 2));
			treasure_ptr->cost += treasure_ptr->p1 * 50000;
		} /* end rand switch */
		break;
				  ...
				  boolean is_cursed, boolean forceit)
{
	if (treasure_ptr->tval == soft_armor && treasure_ptr->subval == 6) {
	...
	} else if (treasure_ptr->tval == hard_armor &&
		   treasure_ptr->subval == 13) {
		...
		if (is_magic || randint(5) == 1) {
			...
			treasure_ptr->weight *= WEIGHT_ADJ;
			treasure_ptr->cost =
			    trunc(treasure_ptr->cost * COST_ADJ);
		}
	}
		...
		if (treasure_ptr->subval == 99 && randint(5) == 1) {
			...
			treasure_ptr->flags |=
			    (Charisma_worn_bit | Searching_worn_bit |
			     ...
			     Resist_Acid_worn_bit | Resist_Cold_worn_bit);
			treasure_ptr->p1 = -5;
			treasure_ptr->cost = 120000;
		      ...
		      boolean forceit)
{
	if (is_magic) {
		...
		case 1:
			...
			treasure_ptr->flags |=
			    (Cursed_worn_bit | Speed_worn_bit);
			treasure_ptr->p1 = -1;
			break;
		case 2:
		     ...
		     boolean is_special, boolean is_cursed, boolean forceit)
{
	...
	case 33: /* Slaying */
		...
		treasure_ptr->cost +=
		    (treasure_ptr->tohit + treasure_ptr->todam) * 10000;
		if (is_cursed) {
			treasure_ptr->flags |= Cursed_worn_bit;
			treasure_ptr->tohit *= -1;
			treasure_ptr->todam *= -1;
...
static void mt__wand(treasure_type *treasure_ptr, boolean forceit)
{
	...
	case 25:
		...
		break;
	default:
		MSG(("WARNING: Unknown subval in mt__wand: %ld  %s",
		     treasure_ptr->subval, treasure_ptr->name));
		p1 = 0;
		break;
	}
...
static void mt__staff(treasure_type *treasure_ptr, boolean forceit)
{
	...
	case 26:
		...
		break;
	default:
		MSG(("WARNING: Unknown subval in mt__staff: %ld  %s",
		     treasure_ptr->subval, treasure_ptr->name));
		p1 = 0;
		break;
	}
...
static void mt__chime(treasure_type *treasure_ptr, boolean forceit)
{
	...
	case 16:
		...
		break;
	default:
		MSG(("WARNING: Unknown subval in mt__chime: %ld  %s",
		     treasure_ptr->subval, treasure_ptr->name));
		p1 = 0;
		break;
	}
...
static void mt__horn(treasure_type *treasure_ptr, boolean forceit)
{
	...
	case 13:
		...
		break;
	default:
		MSG(("WARNING: Unknown subval in mt__horn: %ld  %s",
		     treasure_ptr->subval, treasure_ptr->name));
		p1 = 0;
		break;
	}
		      ...
		      boolean forceit)
{
	if (is_magic) {
		...
		case 1:
			...
			treasure_ptr->ac = 0;
			treasure_ptr->toac =
			    -mt__m_bonus(1, 10, level, forceit);
			treasure_ptr->tohit =
			    -mt__m_bonus(1, 10, level, forceit);
			treasure_ptr->todam =
			    -mt__m_bonus(1, 10, level, forceit);
		     ...
		     boolean forceit)
{
	...
	if (treasure_ptr->tval == bolt || treasure_ptr->tval == arrow) {
		if (is_magic) {
			treasure_ptr->tohit =
			    mt__m_bonus(1, 35, level, forceit);
			treasure_ptr->todam =
			    mt__m_bonus(1, 35, level, forceit);
...
void magic_treasure(integer x, integer level, boolean forceit)
{
	...
	case hard_armor:
	case soft_armor:
		mt__armor_and_shields(treasure_ptr, level, is_magic, is_special,
				      is_cursed, forceit);
		break;
	...
	case dagger:
	case maul:
		mt__weapons(treasure_ptr, level, is_magic, is_special,
			    is_cursed, forceit);
		break;

	case bow_crossbow_or_sling:
		mt__bows_and_slings(treasure_ptr, level, is_magic, is_special,
				    is_cursed, forceit);
		break;
	...
	case gloves_and_gauntlets:
		mt__gloves_and_gauntlets(treasure_ptr, level, is_magic,
					 is_special, is_cursed, forceit);
		break;

	case boots:
		mt__boots(treasure_ptr, level, is_magic, is_special, is_cursed,
			  forceit);
		break;

	case helm:
		mt__helms(treasure_ptr, level, is_magic, is_special, is_cursed,
			  forceit);
		break;

	/*{girdles, belts and buckles}*/
	case belt:
		mt__belt(treasure_ptr, level, is_magic, is_special, is_cursed,
			 forceit);
		break;
	...
	case cloak:
		mt__cloak(treasure_ptr, level, is_magic, is_special, is_cursed,
			  forceit);
		break;
	...
	case bolt:
	case spike:
		mt__ammo(treasure_ptr, level, is_magic, is_special, is_cursed,
			 forceit);
		break;
