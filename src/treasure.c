...
void mt__ego_sword(integer x)
{
	...
	case 1: /*{Holy Avenger}*/
		...
		t_list[x].flags |=
		    (See_Invisible_worn_bit | Sustain_Stat_worn_bit |
		     Resist_Acid_worn_bit | Resist_Fire_worn_bit |
		     Strength_worn_bit | Slay_Undead_worn_bit |
		     Slay_Evil_worn_bit);
	...
	case 2: /*{Defender}*/
		...
		t_list[x].flags |=
		    (Feather_Fall_worn_bit | See_Invisible_worn_bit |
		     ...
		     Resist_Fire_worn_bit | Regeneration_worn_bit |
		     Stealth_worn_bit);
	...
	case 4: /* {Soul Sword}*/
		...
		t_list[x].flags |= (Intelligence_worn_bit | Wisdom_worn_bit |
				    Charisma_worn_bit | See_Invisible_worn_bit |
				    Regeneration_worn_bit);
		t_list[x].flags2 |= (Soul_Sword_worn_bit | Bad_repute_worn_bit);
...
void mt__slaying_sword(integer x)
{
	...
	case 1: /* {Slay Monster}*/
		...
		t_list[x].flags |=
		    (See_Invisible_worn_bit | Slay_Monster_worn_bit);
		t_list[x].tohit += 3;
		t_list[x].todam += 3;
		t_list[x].cost += 500000;
		break;
...
/*//////////////////////////////////////////////////////////////////// */
integer mt__m_bonus(integer base, integer max_std, integer level,
		    boolean forceit)
{
...
/*//////////////////////////////////////////////////////////////////// */
void mt__gems(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	integer p1;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__misc_usable(integer x, integer level, integer chance, integer special,
		     integer cursed, boolean forceit)
{
		...
		if (mt__magik(special) || forceit) {
			...
			case 14: /*{statues}*/
				...
				case 1: /*{summoning undead}*/
					strcat(t_list[x].name,
					       " Major of Undead Summoning");
			   ...
			   integer special, integer cursed, boolean forceit)
{
	if ((t_list[x].tval == soft_armor) && (t_list[x].subval == 6)) {
		...
		if ((randint(4) == 1) || forceit) {
			t_list[x] =
			    yums[11 + randint(3)]; /* various non foods */
		...
		if (mt__magik(chance) || forceit || (randint(5) == 1)) {
			if (mt__magik(special) || (randint(5) == 1)) {
				if (randint(3) == 1) {
					t_list[x] =
					    yums[17]; /* Mithril Plate Armor */
				} else {
					t_list[x] =
					    yums[16]; /* Mithril Chain Mail */
				}
			} else {
...
/*//////////////////////////////////////////////////////////////////// */
void mt__weapons(integer x, integer level, integer chance, integer special,
		 integer cursed, boolean forceit)
{
	integer wpn_type;
	...
	if (mt__magik(chance) || forceit) {
		...
		if (mt__magik(special)) {
			if ((t_list[x].subval == 99) &&
			    (randint(5) == 1)) { /* filthy rags? */
				strcat(t_list[x].name, " of Trollkind");
				t_list[x].flags |=
				    (Charisma_worn_bit | Searching_worn_bit |
				     Stealth_worn_bit | Regeneration_worn_bit |
				     Resist_Acid_worn_bit |
				     Resist_Cold_worn_bit);
				t_list[x].p1 = -5;
				t_list[x].cost = 120000;
	...
	if (mt__magik(chance) || forceit) {
		...
		if (mt__magik(special)) {
			if ((t_list[x].subval == 5) && (randint(10) == 1)) {
				/* gloves_and_gauntlets 5 is "Set of Cloth
				 * Gloves" */
				...
				case 3:
					...
					t_list[x].p1 =
					    mt__m_bonus(5, 50, level, forceit);
					t_list[x].cost +=
					    20000 + t_list[x].p1 * 5;
					break;
				case 4:
		...
		if (mt__magik(special)) {
			...
			case 1:
				...
				t_list[x].flags |=
				    (Cursed_worn_bit | Dexterity_worn_bit);
				t_list[x].p1 = 1;
				break;
			case 2:
				strcat(t_list[x].name, " of Weakness");
				t_list[x].flags |=
				    (Cursed_worn_bit | Strength_worn_bit);
				t_list[x].p1 = 1;
				break;
			case 3:
...
/*//////////////////////////////////////////////////////////////////// */
void mt__boots(integer x, integer level, integer chance, integer special,
	       integer cursed, boolean forceit)
{
	if (mt__magik(chance) || forceit) {
		t_list[x].toac = mt__m_bonus(1, 20, level, forceit);
		if (mt__magik(special)) {
			...
			case 5:
				...
				break;
			default:
				if ((t_list[x].subval == 4) &&
				    (randint(6) == 1)) { /* Pair of Sandals */
		...
		case 1:
			...
			t_list[x].flags |= (Cursed_worn_bit | Speed_worn_bit);
			t_list[x].p1 = -1;
			break;
		case 2:
...
/*//////////////////////////////////////////////////////////////////// */
void mt__helms(integer x, integer level, integer chance, integer special,
	       integer cursed, boolean forceit)
{
	if (mt__magik(chance) || forceit) {
		t_list[x].toac = mt__m_bonus(1, 20, level, forceit);
			...
			case 15:
				...
				t_list[x].flags |=
				    (Cursed_worn_bit | Strength_worn_bit |
				     Dexterity_worn_bit |
				     Constitution_worn_bit |
				     Intelligence_worn_bit | Wisdom_worn_bit |
				     Charisma_worn_bit | Stealth_worn_bit |
				     Aggravation_worn_bit |
				     Teleportation_worn_bit |
				     Blindness_worn_bit | Timidness_worn_bit);
				t_list[x].flags2 |=
				    (Hunger_worn_bit | Known_cursed_bit);
				t_list[x].p1 = -5;
				break;
			} /* end rand switch */
...
/*//////////////////////////////////////////////////////////////////// */
void mt__belt(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	if (mt__magik(chance) || forceit) {
		t_list[x].toac = mt__m_bonus(1, 20, level, forceit);
		...
		if (mt__magik(special)) {
			...
			case 11:
				...
				t_list[x].flags |=
				    (Cursed_worn_bit | Timidness_worn_bit);
				t_list[x].p1 = -1;
				break;
			case 13: /* Leather Belt */
...
/*//////////////////////////////////////////////////////////////////// */
void mt__ring(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	switch (t_list[x].subval) {
	...
	case 7: /* speed */
		if (mt__magik(cursed)) {
			...
			if (mt__magik(special)) {
				if (randint(100) ==
				    1) { /* added a rare, faster ring STK */
					t_list[x].p1 = 2;
					t_list[x].cost *= 10;
				}
			}
	...
	case 23: /*	{ Increase To-Hit	}*/
		...
		t_list[x].cost +=
		    t_list[x].tohit * 10000; /* XXXX seems like tohit... */
		/* yes indeed.  looks like cut-and-paste error.  fixed 10/18/00
		 * --JEB */
		if (mt__magik(cursed)) {
			t_list[x].flags |= Cursed_worn_bit;
			t_list[x].tohit *= -1;
	...
	case 24: /*	{ Protection		}*/
		t_list[x].toac = mt__m_bonus(1, 20, level, forceit);
		t_list[x].cost +=
		    t_list[x].toac * 10000; /* XXXX seems like toac... */
		/* yes indeed.  looks like cut-and-paste error.  fixed 10/18/00
		 * --JEB */
		if (mt__magik(cursed)) {
			t_list[x].flags |= Cursed_worn_bit;
	...
	case 35: /*   { Speed -10 or worse }*/
		...
		/* these rings weren't actually cursed; the original code looks
		 */
		/* like it was */
		/* trying to check the cursed_worn_bit and clear the */
		/* Known_cursed_bit if */
		/* the ring was cursed, but it was actually clearing the */
		/* cursed_worn bit */
		/* (i.e. used flags vs. flags2).  added code to set */
		/* cursed_worn_bit (which */
		/* wasn't happening anywhere) and to unset Known_cursed_bit for
		 */
		/* real. */
		/* 2/15/00 JEB */
		if (mt__magik(cursed)) {
			...
			/*      t_list[x].flags &= 0x7FFFFFFF; * XXXX what does
			 */
			/*      this bit mean? */
		}
		break;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__amulet(integer x, integer level, integer chance, integer special,
		integer cursed, boolean forceit)
{
	switch (t_list[x].subval) {
...
/*//////////////////////////////////////////////////////////////////// */
void mt__wand(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	integer p1;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__staff(integer x, integer level, integer chance, integer special,
	       integer cursed, boolean forceit)
{
	integer p1;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__chime(integer x, integer level, integer chance, integer special,
	       integer cursed, boolean forceit)
{
	integer p1;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__horn(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	integer p1;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__cloak(integer x, integer level, integer chance, integer special,
	       integer cursed, boolean forceit)
{
	if (mt__magik(chance) || forceit) {
		if (mt__magik(special)) {
			...
			case 4:
				...
				t_list[x].toac =
				    mt__m_bonus(2, 40, level, forceit);
				t_list[x].cost +=
				    25000 + t_list[x].toac * 10000;
				break;
			case 5:
			case 6:
			case 7:
			case 8:
				...
				t_list[x].flags |= Stealth_worn_bit;
				t_list[x].toac =
				    mt__m_bonus(1, 20, level, forceit);
				t_list[x].p1 = randint(3);
				t_list[x].cost += t_list[x].p1 * 50000 +
						  t_list[x].toac * 10000;
				break;
			case 9:
				...
				t_list[x].flags |=
				    (See_Invisible_worn_bit |
				     Sustain_Stat_worn_bit | Stealth_worn_bit |
				     Charisma_worn_bit);
				t_list[x].p1 = 2;
				t_list[x].cost += 200000;
				break;
		...
		case 2:
			...
			t_list[x].ac = 0;
			t_list[x].toac =
			    -mt__m_bonus(10, 100, level + 50, forceit);
			t_list[x].cost = 0;
			break;
		case 3:
			...
			t_list[x].toac = -mt__m_bonus(1, 10, level, forceit);
			t_list[x].tohit =
			    -mt__m_bonus(2, 40, level + 10, forceit);
			t_list[x].todam =
			    -mt__m_bonus(2, 40, level + 10, forceit);
			t_list[x].cost = 0;
			break;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__chest(integer x, integer level, integer chance, integer special,
	       integer cursed, boolean forceit)
{
	/*
...
/*//////////////////////////////////////////////////////////////////// */
void mt__ammo(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	integer i1;
...
/*//////////////////////////////////////////////////////////////////// */
void mt__food(integer x, integer level, integer chance, integer special,
	      integer cursed, boolean forceit)
{
	if (mt__magik(chance) || forceit) {
		if (mt__magik(special)) {
			if (mt__magik(cursed)) {
				if (t_list[x].subval ==
				    319) { /* Eyeballs of Unhealth */
					t_list[x] =
					    yums[18]; /* Eyeballs of Drong */
					t_list[x].weight *= WEIGHT_ADJ;
					t_list[x].cost *= COST_ADJ;
				}
			}
...
/*//////////////////////////////////////////////////////////////////// */
void mt__get_chances(integer level, integer *chance, integer *special,
		     integer *cursed, boolean forceit)
{
	...
	*chance =
	    obj_base_magic + (level * (obj_base_max - obj_base_magic))div 100;
...
void magic_treasure(integer x, integer level, boolean forceit)
{
	...
	case hard_armor:
	case soft_armor:
		mt__armor_and_shields(x, level, chance, special, cursed,
				      forceit);
		break;
	...
	case gloves_and_gauntlets:
		mt__gloves_and_gauntlets(x, level, chance, special, cursed,
					 forceit);
		break;
