...
{
	prt("You may:", 21, 1);
	prt(" p) Bid on an item.            <space> browse store's inventory.",
	    22, 1);
	prt(" s) Put an item up for bid.    i) Inventory and Equipment Lists.",
	    23, 1);
		  ...
		  trade_record_type *profits)
{
	...
	*cur_top = NULL;
	...
	while (!feof(sales) && !done) {
		item = (pinven_ptr)safe_malloc(sizeof(pinven_record),
					       "tp__read_inv");
		got = read((int)fileno(sales), &(item->data),
			   sizeof(trade_record_type));
		if (got == 0) {
			done = true;
			dispose(item, sizeof(inven_record),
				"tp__read_inv: read 0");
			...
			msg_print("");
			dispose(item, sizeof(inven_record),
				"tp__read_inv: bad read");
		} else {
			if (item->data.pr.trade_type == TT_PROFIT) {
				profits->pr.money = item->data.pr.money;
				dispose(item, sizeof(trade_record_type),
					"tp__read_inv: profit type");
			} else if (first) {
				item->prev = NULL;
				*inv = item;
				*cur_top = item;
		   ...
		   integer *cur_display_size, pinven_ptr cur_display[])
{
	...
	*blegga = NULL;
	...
	while (item != NULL) {
		if (!panic) {
			if (fwrite(&(item->data), sizeof(trade_record_type), 1,
				   sales) != 1) {
				panic = true;
		       ...
		       trade_record_type *profits, boolean *redisplay)
{
	...
	(*item)->data.fsr.best_bid =
	    ROUND((*item)->data.fsr.best_bid * T_BID_INCREMENT_FACTOR *
		  T_REFUND_ON_SALE);
	tp__send_money(&((*item)->data.fsr.seller),
		       ROUND(T_REFUND_ON_SALE * (*item)->data.fsr.best_bid),
		       inv);
	tp__made_profit(item_value(&((*item)->data.fsr.object)) -
			    (*item)->data.fsr.best_bid,
			profits);
	*redisplay = true;
	tp__delete_item(item, inv,
			cur_top); /* XXXX safe to do? make item a **? */
}
/*//////////////////////////////////////////////////////////////////// */
			  ...
			  boolean *exit_flag)
{
	...
	if (inven_check_num() && inven_check_weight()) {
		...
		tp__made_profit(
		    ROUND(T_PROFIT_FROM_SALE * (*item)->data.fsr.best_bid),
		    profits);
		...
		if (tp__enough_time(item->data.fsr.time, T_EXPIRE_TIME_DAYS,
			...
			if ((item->data.fsr.best_bidder.claim_check ==
				...
				if (item->data.fsr.best_bid <
				    ROUND(
					T_TAKE_THE_MONEY_AND_RUN *
					item_value(&(item->data.fsr.object)))) {
					tpd__post_outbids(&item, inv, cur_top,
							  profits, &redisplay);
				} else {
					tpd__player_wins_bid(
					    &item, inv, cur_top, profits,
					    &redisplay, &weight_changed,
					    exit_flag);
				}
	     ...
	     boolean *exit_flag)
{
	...
	if (*cur_display_size > 0) {
		if (tp__get_store_item(&which, "Which one?", 1,
				       *cur_display_size)) {
	      ...
	      pinven_ptr cur_display[], vtype shop_owner)
{
	...
	if (get_item(&item_ptr, "Which one? ", &redraw, inven_ctr, &trash_char,
		     false, false)) {
		wgt = 0;
		...
		if ((strstr(item_ptr->data.name, "|") != NULL) ||
			...
			strcpy(response,
			       "I can't sell that!  Identify it first!");
		} else if (wgt != 0) {
			strcpy(
			    response,
			    "Hey that bag is full of items! Empty it first.");
		} else if (item_ptr->is_in) {
			strcpy(response,
			       "You can't sell an item *IN* a bag of holding.");
		} else if (item_value(&(item_ptr->data)) <
			   T_ACCEPTABLE_ITEM_PRICE) {
			strcpy(response,
			       "What is THAT?  I won't have that in my shop!");
		} else if ((item_ptr->data.flags2 & Blackmarket_bit) != 0) {
			strcpy(
			    response,
			    "Hmmm, I don't think I want that on the shelves.");
		} else {
			item = (pinven_ptr)safe_malloc(sizeof(pinven_record),
						       "tp__sell");
			item->next = *inv;
		...
		if (tp__get_store_item(&which, "Delete which one?", 1,
			...
			if (get_com("Refund money? (Y/N)", &command)) {
				...
				case 'Y':
					...
					tp__send_money(
					    &(cur_display[which]
						  ->data.fsr.best_bidder),
					    ROUND(T_REFUND_ON_BID *
						  cur_display[which]
						      ->data.fsr.best_bid),
					    inv);
					tp__made_profit(
					    ROUND(T_PROFIT_FROM_BID *
						  cur_display[which]
						      ->data.fsr.best_bid),
					    profits);
					tp__send_money(
				...
				tp__delete_item(&(cur_display[which]), inv,
						cur_top);
				cur_display[which] = NULL;
				tp__display_inv(*cur_top, inv, blegga,
						cur_display_size, cur_display);
			}
		}
	}
		  ...
		  integer *cur_display_size, pinven_ptr cur_display[])
{
	...
	if (*cur_display_size > 0) {
		if (tp__get_store_item(&which, "Info on which?", 1,
			...
			erase_line(12 + 6, 1);
			sprintf(out_val, "%ld",
				cur_display[which]->data.fsr.time);
			...
			sprintf(
			    ...
			    cur_display[which]
				->data.fsr.best_bidder.claim_check);
		       ...
		       trade_account_type *cur_player, boolean *exit_flag)
{
	...
	if (get_com("", &command)) {
		...
		case 4: /* ^d */
			if (wizard2) {
				tp__nuke_item(inv, cur_top, blegga,
					      cur_display_size, cur_display,
					      profits);
			}
			break;
		...
		case 9: /* ^i */
			if (wizard2) {
				tp__get_info(inv, blegga, cur_top,
					     cur_display_size, cur_display);
			}
			break;
		...
		case 16: /* ^p */
			if (wizard2) {
				sprintf(out_val, "Profits made to date: %ld",
					profits->pr.money);
				msg_print(out_val);
				msg_print("");
		...
		case 101:
			if (inven_command('e', &trash_ptr, "")) {
				tp__display_store(shop_owner, inv, blegga,
						  cur_top, cur_display_size,
						  cur_display);
			}
			break;
		...
		case 112:
			if (PM.cheated) {
				...
				tp__bid(cur_display_size, cur_player, inv,
					cur_top, blegga, cur_display, profits,
					exit_flag);
			}
			break;
		...
		case 115:
			if (PM.cheated) {
				...
				tp__sell(inv, cur_top, blegga, cur_player,
					 cur_display_size, cur_display,
					 shop_owner);
			}
			break;
...
void enter_trading_post()
{
	...
	if (!exit_flag) {
		...
		if (!exit_flag) {
			...
			for (; !exit_flag;) {
				tp__parse_command(
				    &inv, &cur_top, &blegga, &cur_display_size,
				    cur_display, &profits, shop_owner,
				    &cur_player, &exit_flag);
				adv_time(false);
				tics++;
