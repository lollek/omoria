			 ...
			 statr odds, statr true_odds, long *true_sum_bets)
{
	...
	h_stat horsestat = {99, 25, 30, 28, 22, 21, 24, 23, 22, 23, 20, 26,
			    ...
			    26, 22, 25, 21, 22, 26, 24, 28, 27, 29, 29, 28,
			    26, 26, 17, 25, 18, 26, 31, 28, 23, 28, 26, 25,
			    27, 29, 25, 23, 26, 27, 26, 27};
...
void hr__list_horses(long num_horse, h_name horse, statr odds)
{
...
void s__get_positions(long num_horse, long win_count, h_stat dist,
...
boolean s__find_plot(long horse, long *row, long *col, h_stat dist,
		     long screen_low, long screen_high, h_stat lane)
{
	...
	if ((t >= screen_low) && (t <= screen_high)) {
		...
		if (t <= 730) {
			...
			*row = 3 + (int)(cos((t - 1090) / 140) *
					 (120 + 10 * lane[horse]) / 15);
		} else if (t <= 1530) {
			*row = 23 + (int)(cos((t - 1090) / 140) *
					  (120 + 10 * lane[horse]) / 15);
		} else if (t <= 2970) {
			*row = 14 - lane[horse] / 2;
		} else if (t <= 3190) {
			*row = 23 - (int)(cos((t - 2970) / 140) *
					  (120 + 10 * lane[horse]) / 15);
		} else {
			*row = 3 - (int)(cos((t - 2970) / 140) *
					 (120 + 10 * lane[horse]) / 15);
		}
		...
		if (t <= 730) {
			...
			*col = 37 + (int)(sin((t - 1090) / 140) *
					  (120 + lane[horse] * 10) / 10);
		} else if (t <= 1890) {
			*col = 190 - t / 10;
		} else if (t <= 2610) {
			*col = 262 - t / 10;
...
void s__predict_order(long num_horse, statr true_odds, h_stat final_order,
...
void s__predict_race(long num_horse, h_stat h_times[], h_stat final_fastest,
		     h_stat hstat)
{
	...
	for (i1 = 1; i1 <= num_horse; i1++) {
		...
		for (i2 = 1; i2 <= 6; i2++) {
			try_speedy =
			    ((randint(t2) + randint(t2) + randint(t2)) / 100 +
			     2 * (num_horse - i1 + 1));
			if ((try_speedy > speedy) &&
			    (try_speedy < old_speedy)) {
				speedy = try_speedy;
			}
		}
		...
		for (i2 = 1; i2 <= 20; i2++) {
			h_times[i2][t] =
			    31 - speedy / 10; /*{time to move distance x}*/
		}
...
void hr__start(long num_horse, long *screen_high, long *which_screen,
	...
	for (*which_screen = 1; *which_screen <= 7; (*which_screen)++) {
		*screen_low = *screen_high + 1;
		...
		do {
			count += 2;
			for (i1 = 1; i1 <= num_horse; i1++) {
				if (s__find_plot(i1, &row, &col, dist,
						 *screen_low, *screen_high,
						 lane)) {
					put_buffer(" ", row, col);
				}
				if ((count > h_times[area[i1]][i1]) &&
				    (area[i1] < 21)) {
					area[i1] += 1;
				}
				dist[i1] =
			...
			if (*which_screen == 7) {
				if (*new_win_count > *win_count) {
					s__get_positions(num_horse, *win_count,
							 dist, fastest, order);
...
boolean grb__get_bet_horse(long num_horse, h_name horse, statr odds,
			   long *horse_bet)
{
	boolean exit_flag = false;
	vtype comment;
			      ...
			      h_stat true_bets, statr true_odds)
{
	...
	if (get_com("", &command)) {
		switch (command) {
		case 112:
			return_value = hr__get_race_bet(
			    horse_bet_type, horse_bet, *num_horse, horse, odds,
			    bets, sum_bets);
			break;
		case 18: /*(*^R*)*/
			clear_screen();
		...
		case 118:
			...
			break;
		case 115:
			hr__get_horse_stats(num_horse, horse, hstat, sum_bets,
					    bets, true_bets, odds, true_odds,
					    true_sum_bets);
...
void hr__game_horse()
{
	...
	switch (randint(4)) {
	case 4:
		prt("It is pouring down rain and the track is muddy.", 1, 1);
		break;
	default:
		prt("It is a beautiful day at the track.", 1, 1);
		break;
	}
	...
	do {
		...
		if (hr__get_horse_command(&exit_flag, &horse_bet_type,
					  &horse_bet, &sum_bets, &num_horse,
					  horse, odds, bets, &true_sum_bets,
					  hstat, true_bets, true_odds)) {
			hr__start(num_horse, &screen_high, &which_screen,
				  &win_count, &new_win_count, &screen_low,
				  true_odds, final_order, final_fastest,
				  fastest, hstat, area, dist, order, &horse_bet,
				  horse, lane);
			hr__display_winnings(horse_bet, fastest, odds,
					     horse_bet_type, horse);
			spend_time(4800 / RACES_PER_DAY, "at the track", false);
			hr__display_commands();
			c__display_gold();
			hr__get_horse_stats(&num_horse, horse, hstat, &sum_bets,
		...
		if (((player_cur_age.day % 7) >= 2) &&
			...
			c_closed = true;
			exit_flag = true;
			msg_print("There are no more races today...Come back "
				  "tomorrow.");
			msg_print(" ");
		}
	} while (!exit_flag);
