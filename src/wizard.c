...
boolean wizard_moo_item(treasure_type *back) {
  ...
  *back = blank_treasure;
  ...
  do {
    flag = wmi__parse_command(&blegga, &cur_top, &cur_display_size, &exit_flag,
                              back, cur_display, &data_list);
  } while (!exit_flag);
...
boolean summon_item(long y, long x, char name1[70], char name2[70], long count,
  ...
  if (flag) {
    si__pesky_stuff(&best_value, &good_value, &best_pick, &good_pick, &optimize,
                    out_str, &i_summ_count, &cur_pos, x, y);
