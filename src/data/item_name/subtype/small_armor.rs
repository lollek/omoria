...
use crate::model::item_subtype::{
    BeltSubType, BootsSubType, BracersSubType, CloakSubType, GlovesSubType, HelmSubType,
    ItemSubType,
};
...
pub fn small_armor(item: &Item) -> String {
    ...
    if item.is_identified()
        && subtype == ItemSubType::Bracers(BracersSubType::SilverBraceletOfWarding)
    {
...
fn subtype_name(item: &Item, subtype: ItemSubType) -> String {
    match subtype {
        ...
        ItemSubType::Bracers(bracers_type) => match bracers_type {
            ...
            BracersSubType::BracersOfDefense => {
                if item.is_identified() {
                    "bracers of defense"
                } else {
                    "bracers"
            ...
            BracersSubType::BracersOfShielding => {
                if item.is_identified() {
            ...
            BracersSubType::BracersOfWeaponAttraction => {
                if item.is_identified() {
            ...
            BracersSubType::SilverBraceletOfWarding => {
                if item.is_identified() {
    ...
    fn test_identified() {
        let tests: Vec<(Box<dyn ItemTemplate>, i16, i64, &str)> = vec![
            ...
            (
                ...
                1,
                0,
                "bracers of protection [1,+1]",
            ),
            (
                Box::new(BracersTemplate::BracersOfDefense),
                -1,
                0,
                "bracers of defense [-1,-1]",
            ),
            (
                Box::new(BracersTemplate::BracersOfShielding),
                1,
                0,
                "bracers of shielding [1,+1]",
            ),
            (
                Box::new(BracersTemplate::BracersOfWeaponAttraction),
                -1,
                0,
                "bracers of weapon attraction [-1,-1]",
            ),
            (
                Box::new(BracersTemplate::SilverBraceletOfWarding),
                1,
                0,
                "silver bracelet of warding [1,+1] (R)",
            ),
            (
                Box::new(BracersTemplate::SilverBracelet),
                1,
