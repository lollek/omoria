...
use crate::data::item_name::helpers::{maybe_armor_bonus, maybe_p1_bonus, to_hit_if_exists};
...
mod tests {
    ...
    fn test_unidentified() {
        let tests: Vec<(Box<dyn ItemTemplate>, &str)> = vec![
            (Box::new(ArmorTemplate::AugmentedChainMail), "augmented chain mail"),
            (Box::new(ArmorTemplate::BarChainMail), "bar chain mail"),
            (Box::new(ArmorTemplate::BronzePlateMail), "bronze plate mail"),
            (Box::new(ArmorTemplate::ChainMail), "chain mail"),
            (Box::new(ArmorTemplate::CoolSetOfThreads), "cool set of threads"),
            (Box::new(ArmorTemplate::DemonhideArmor), "demonhide armor"),
            (Box::new(ArmorTemplate::DoubleChainMail), "double chain mail"),
            (Box::new(ArmorTemplate::DuskShroud), "dusk shroud"),
            (Box::new(ArmorTemplate::ElvenChainMail), "elven chain mail"),
            (Box::new(ArmorTemplate::FilthyNagaHideArmor), "filthy naga hide armor"),
            (Box::new(ArmorTemplate::FilthyRags), "filthy rags"),
            (Box::new(ArmorTemplate::FullPlateArmor), "full plate armor"),
            (Box::new(ArmorTemplate::HardLeatherArmor), "hard leather armor"),
            (Box::new(ArmorTemplate::HardLeatherRingMail), "hard leather ring mail"),
            (Box::new(ArmorTemplate::HardStuddedLeather), "hard studded leather"),
            (Box::new(ArmorTemplate::LacqueredPlate), "lacquered plate"),
            (Box::new(ArmorTemplate::LaminatedArmor), "laminated armor"),
            (Box::new(ArmorTemplate::LeatherScaleMail), "leather scale mail"),
            (Box::new(ArmorTemplate::MetalBrigandineArmor), "metal brigandine armor"),
            (Box::new(ArmorTemplate::MetalLamellarArmor), "metal lamellar armor"),
            (Box::new(ArmorTemplate::MetalScaleMail), "metal scale mail"),
            (Box::new(ArmorTemplate::MithrilChainMail), "mithril chain mail"),
            (Box::new(ArmorTemplate::MithrilPlateArmor), "mithril plate armor"),
            (Box::new(ArmorTemplate::PartialPlateArmor), "partial plate armor"),
            (Box::new(ArmorTemplate::Robe), "robe"),
            (Box::new(ArmorTemplate::RustyChainMail), "rusty chain mail"),
            (Box::new(ArmorTemplate::SoftLeatherArmor), "soft leather armor"),
            (Box::new(ArmorTemplate::SoftLeatherRingMail), "soft leather ring mail"),
            (Box::new(ArmorTemplate::SoftStuddedLeather), "soft studded leather"),
            (Box::new(ArmorTemplate::StonePlateArmor), "stone plate armor"),
            (Box::new(ArmorTemplate::WovenCordArmor), "woven cord armor"),
            (Box::new(ArmorTemplate::WyrmhideArmor), "wyrmhide armor"),
            (Box::new(ArmorTemplate::LeatherBrigantineArmor), "leather brigantine armor"),
        ];
    ...
    fn test_known() {
        let tests: Vec<(Box<dyn ItemTemplate>, &str)> = vec![
            (Box::new(ArmorTemplate::AugmentedChainMail), "augmented chain mail (-2) [1]"),
        ];
        for (template, expected_name) in tests {
            let subtype = template.subtype();
            let mut item = generate_item::generate(template, 0);
    ...
    fn test_identified() {
        let tests: Vec<(Box<dyn ItemTemplate>, i16, i16, &str)> = vec![
            (Box::new(ArmorTemplate::AugmentedChainMail), 1, 1, "augmented chain mail (-2) [1,+1]"),
            (Box::new(ArmorTemplate::AugmentedChainMail), 1, 0, "augmented chain mail (-2) [1,0]"),
            (Box::new(ArmorTemplate::AugmentedChainMail), 1, -1, "augmented chain mail (-2) [1,-1]"),
            (Box::new(ArmorTemplate::AugmentedChainMail), 1, 1, "augmented chain mail (-2) [1,+1]"),
            (Box::new(ArmorTemplate::AugmentedChainMail), 1, 1, "augmented chain mail (-2) [1,+1]"),
        ];
