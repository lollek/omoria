		...
		if (cave[y][x].cptr == 0) {
			if (find_range(pick_a_spike, false, &i1, &i2)) {
				...
				t_list[cave[y][x].tptr].p1 =
				    -labs(t_list[cave[y][x].tptr].p1) - 20;
			} else {
		...
		if (cave[y][x].cptr > 1) {
			if (m_list[cave[y][x].cptr].ml) {
				...
				if (is_vowel(c_list[i2].name[0])) {
					sprintf(out_val, "You see an %s.",
						c_list[i2].name);
				} else {
					sprintf(out_val, "You see a %s.",
						c_list[i2].name);
				}
				msg_print(out_val);
				flag = true;
...
static void d__sun_rise_or_set()
{
	...
	if (dun_level == 0) {
		if ((player_cur_age.hour == 6) && (player_cur_age.secs == 0)) {
			...
			store_maint();
			draw_cave();
		} else if ((player_cur_age.hour == 18) &&
...
static void d__check_food()
{
	...
	if (((player_flags).hunger_item) && ((player_flags).foodc > (PLAYER_FOOD_ALERT + 15))) {
...
static void d__eat_food()
{
	...
	if ((player_flags).speed < 0) {
		(player_flags).foodc -= ((player_flags).speed * (player_flags).speed) + (player_flags).food_digested;
	} else {
...
static void d__update_petrify()
{
	...
	*/
	...
	if ((player_flags).paralysis > 0) {
		...
		if ((search_flag) && ((player_flags).paralysis > (player_flags).paral_init)) {
			search_off();
		}
	}
...
static void d__bash()
{
	...
	if (cave[y][x].cptr > 1) {
		if (player_flags.afraid > 0) {
			...
			player_bth =
			    trunc((((C_player_get_stat(STR) * 10) + 20) / 5 +
				   player_wt) /
				  6.0);
			player_ptohit = 0;
		...
		if (t_list[cave[y][x].tptr].tval == closed_door) {
			...
			if (test_hit(player_wt + (from_str * from_str) / 500, 0,
				     0,
	...
	if (d__get_dir("Which direction?", &tdir, &i1, &y, &x)) {
		...
		if (cave[y][x].tptr > 0) {
			...
			t3 = 2 * C_player_disarm_from_dex(); /*{ Dexterity
								adjustment  }*/
			t4 = C_player_mod_from_stat(
			    INT); /*{ Intelligence adjustment}*/
			tot = t1 + t2 + t3 + t4;
...
void change_rep(long amt)
{
	...
	if ((amt < 0) ||
		...
		player_rep = trunc(
		    sqrt((20 + player_rep) * (20 + player_rep) + 40 * amt) -
		    20);
	}
}
...
long react(long x)
{
	...
	long ans = (C_player_get_stat(CHR) * 10 + (player_rep * 2) +
		    randint(200) + randint(200) + randint(200)) /
		       50 +
		   x - 4;
...
void battle_game(long plus, char kb_str[82])
{
	...
	if (get_yes_no("Do you accept their invitation?")) {
		...
		/* with player_do; */
		for (i1 = 1; i1 <= 7; i1++) {
			if (player_test_hit(player_bth, player_lev, plus,
					    20 * i1, false)) {
				score++;
				time = time * 2 + 10;
			}
		...
		case 5:
			msg_print(
			    "They are amazed by your incredible prowess!");
			change_rep(2);
			break;
...
void eat_the_meal()
{
	...
	default:
		if ((yummers > 0) ||
		    player_saves(player_lev +
				 5 * C_player_mod_from_stat(CON))) {
			msg_print(
...
void worship()
{
	...
	if (get_yes_no("Do you accept?")) {
		...
		case 1:
			msg_print(
			    "You sit through a fascinating church service.");
			break;
		case 2:
			msg_print(
			    "You sit through an interesting church service.");
			break;
		case 3:
		...
		case 4:
			msg_print(
			    "You sit through a long, boring church service.");
			break;
		}

		spend_time(100 * (randint(7) + preachy * preachy),
			   "at the Church", false);
		...
		if (get_yes_no("Will you give him some money?")) {
			...
			if (player_money[TOTAL_] > 0) {
				...
				i1 = ((randint(12) * player_money[TOTAL_]) /
					  1000 +
				      20) *
				     GOLD_VALUE;
				if (i1 >
				    player_money[TOTAL_] * GOLD_VALUE / 2) {
					i1 = player_money[TOTAL_] * GOLD_VALUE /
					     2;
				}
...
void beg_money()
{
	...
	if (get_yes_no("Will you give them some?")) {
		...
		if (player_money[TOTAL_] > 0) {
			...
			i1 =
			    ((randint(12) * player_money[TOTAL_]) / 1000 + 20) *
			    GOLD_VALUE;
...
void xp_loss(long amount)
{
	...
	for (i1 = 1;
	     (long)(exp_per_level[i1] * player_expfact) <= player_exp;) {
		i1++;
	}
	i2 = player_lev - i1;
		...
		if (C_player_uses_magic(M_ARCANE) ||
			...
			if (flag) {
				...
				if (C_player_uses_magic(M_ARCANE)) {
					...
					msg_print(
					    "You have forgotten a discipline!");
				}
			}
		}
	}
	...
	if (mean_jerk_flag) {
		...
		do {
			if (player_test_hit(player_bth, player_lev, tot_tohit,
				...
				if (equipment[Equipment_primary].tval > 0) {
					i3 =
					    damroll(equipment[Equipment_primary]
							.damage);
					i3 = tot_dam(
					    &equipment[Equipment_primary], i3,
					    &c_list[a_mptr]);
					is_sharp =
				...
				if (is_in(equipment[Equipment_primary].tval,
					...
					if (equipment[Equipment_primary]
						.number <= 0) {
						inven_weight -=
						    equipment[Equipment_primary]
							.weight;
...
long tot_dam(treasure_type *item, long tdam, creature_type *monster)
{
	...
	obj_set stuff_that_goes_thump = {sling_ammo,
					 bolt,
					 arrow,
					 lamp_or_torch,
					 bow_crossbow_or_sling,
	...
	do {
		...
		if (i3 > 0) {
			...
			if ((C_magic_spell_level(i3) <= player_lev) &&
				...
				aspell[i1++].splnum =
				    -1; /* leave gaps for unknown spells */
			}
		}
...
void dungeon()
{
	...
	do {
		...
		if (((player_flags).speed > 0) || ((turn % (labs((player_flags).speed) + 1)) == 0)) {
			water_move();
		...
		if (randint(MAX_MALLOC_CHANCE) == 1) {
			alloc_land_monster(floor_set, 1, MAX_SIGHT, false,
					   false);
		}
		...
		if ((player_flags.paralysis < 1) && /*{ Accept a command? }*/
			...
			do {
			    ...
			    !(!reset_flag || moria_flag)); /* end command do */
