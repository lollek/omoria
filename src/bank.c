...
static void eb__withdraw_money()
{
	...
	if (withdraw > 0) {
		...
		for (mon_type = MITHRIL; mon_type >= GOLD; mon_type--) {
			amt_given[mon_type] =
			    min3((withdraw * GOLD_VALUE) / coin_value[mon_type],
		...
		deliver = true;
		is_some = ((amt_given[MITHRIL] + amt_given[PLATINUM] +
			    amt_given[GOLD]) > 0);
		...
		if (withdraw > 0) {
			...
			if (weight_left <= 0) { /*{ they can't carry it all }*/
				if (is_some) {
					...
					deliver =
					    get_yes_no("Do you want as much as "
						       "you can carry?");
				} else {
					msg_print(
					    "You cannot carry any more money.");
					deliver = false;
				}
		...
		if (deliver) {
			for (mon_type = MITHRIL; mon_type >= GOLD; mon_type--) {
				if (amt_given[mon_type] > 0) {
					...
					player_account -= amt_given[mon_type] *
							  coin_value[mon_type] /
							  GOLD_VALUE;
				}
			} /* end for */
...
static void eb__change_money()
{
	...
	if (change_flag) {
		...
		key_in =
		    (char)get_money_type("Change to? ", &change_flag, true);
	}
	if (change_flag) {
	...
	*exit_flag = false;
	...
	if (get_com(" ", &command)) {
		...
		case 'c':
			...
			break;
		case 'i':
			prt("The insurance shop has gone out of business.", 1,
			    1);
			break;
		case 'p':
