...
void store_maint()
{
	...
	for (; stores[S_BLACK_MARKET].store_ctr > 0;) {
		store_destroy(S_BLACK_MARKET, stores[S_BLACK_MARKET].store_ctr,
			      false);
	}
	...
	for (i1 = 0; i1 < MAX_STORES; i1++) {
		...
		if (store_ctr > STORE_MAX_INVEN) {
			/* too much stuff */
			for (i2 = 0, cnt = (store_ctr - STORE_MAX_INVEN + 2);
			     i2 < cnt; i2++) {
				store_destroy(i1, randint(stores[i1].store_ctr),
					      false);
			}
		} else if (store_ctr < STORE_MIN_INVEN) {
			/* make new stuff */
			for (i2 = 0, cnt = (STORE_MIN_INVEN - store_ctr + 2);
			     i2 < cnt; i2++) {
				store_create(i1);
			}
		} else {
			/* make some, ruin some */
			for (i2 = 0, cnt = (1 + randint(STORE_TURN_AROUND));
			     i2 < cnt; i2++) {
				store_destroy(i1, randint(stores[i1].store_ctr),
					      true);
			}
...
void store_create(long store_num)
{
	...
	do {
		if (store_num != S_BLACK_MARKET) {
			i1 =
			    store_choice[store_num][randint(STORE_CHOICES) - 1];
		...
		if (store_check_num(store_num)) {
			...
			if (t_list[cur_pos].cost >
			    0) { /*{ Item must be good	}*/
				if (t_list[cur_pos].cost <
				    (owners[stores[store_num].owner].max_cost *
				     GOLD_VALUE)) {
...
void store_destroy(long store_num, long item_val, boolean one_of)
{
	...
	if ((stores[store_num].store_inven[item_val].sitem.number > 1) &&
		...
		for (i2 = item_val; i2 < store_ctr; i2++) {
			stores[store_num].store_inven[i2] =
			    stores[store_num].store_inven[i2 + 1];
		}
	...
	*ipos = 0;
	...
	if (icost > 0) {
		...
		do {
		    ...
		    !((item_val >= stores[store_num].store_ctr) || (flag)));
		if (!(flag)) { /*{ Becomes last item in list	}*/
			sc__insert(store_num, stores[store_num].store_ctr + 1,
				   icost);
...
long sell_price(long snum, long *max_sell, long *min_sell, treasure_type *item)
{
	...
	if (item->cost > 0) {
		...
		*max_sell =
		    trunc(i1 * (1 + owners[stores[snum].owner].max_inflate));
		*min_sell =
		    trunc(i1 * (1 + owners[stores[snum].owner].min_inflate));
...
long item_value(treasure_type *item)
{
	...
	case soft_armor:
		...
		if (strstr(item->name, "^") != NULL) {
			return_value =
			    iv__search_list(item->tval, item->subval) *
			    item->number;
		} else {
			switch (item->tval) {
			case bow_crossbow_or_sling:
	...
	case spike:
		...
		if (strstr(item->name, "^") != NULL) {
			return_value =
			    iv__search_list(item->tval, 1) * (item->number);
		} else {
			if (item->tohit < 0) {
				return_value = 0;
			} else if (item->todam < 0) {
...
boolean check_store_hours(long st, long sh)
{
	...
	if (flag) {
		...
		case 'D':
			...
			if (wizard2) {
				msg_print(
				    "Being a wizard, you break into the shop.");
				msg_print("");
				return_value = true;
			} else {
				sprintf(out_val,
			...
			if (get_yes_no(prop)) {
				if (player_money[TOTAL_] >= store_bribe[st]) {
					subtract_money(store_bribe[st] *
							   GOLD_VALUE,
						       false);
					msg_print("The owner reluctantly lets "
						  "you in.");
					msg_print("");
					return_value = true;
				} else {
					return_value = false;
					msg_print("You haven't the money to "
						  "bribe the owner!");
				}
			} else {
				return_value = false;
...
void display_commands()
{
	...
	prt(" p/P) Purchase an item.          <space> browse store's "
	    "inventory.",
	    22, 1);
	prt(" s/S) Sell an item.              i) Inventory and Equipment "
	    "Lists.",
	    23, 1);
	...
	for (; (start <= stop);) {
		...
		if (stores[store_num].store_inven[start].scost < 0) {
			...
			sprintf(out_val2, "%6ld [Fixed]",
				(stores[store_num].store_inven[start].scost /
				 GOLD_VALUE));
		}
	...
	if (!find_range(store_buy[store_num], false, &item_ptr, &count)) {
		...
		if (is_in(inven_temp->data.tval, store_buy[store_num])) {
			if (store_check_num(store_num)) {
				...
				switch (sell_haggle(store_num, &price,
						    &(inven_temp->data),
						    blitz)) {
				case 0:
					prt_comment1();
				...
				case 4:
					/* black market or insured stuff after a
					 * death */
					msg_print("Hmmmmm, that looks hot. I "
						  "will not buy it!");
				...
				prt("I have not the room in my store to keep "
				    "it...",
				    1, 1);
			}
		} else {
...
boolean get_store_item(long *com_val, vtype pmt, long i1, long i2)
{
	...
	*com_val = 0;
	sprintf(out_val, "(Items %c-%c, Esc to exit) %s", (int)i1 + 96,
		(int)i2 + 96, pmt);
		 ...
		 boolean blitz)
{
	...
	*price = 0;
	...
	if (cost < 1) {
		...
		max_sell = trunc(
		    cost * (1 + (owners[stores[store_num].owner]).max_inflate));
		max_buy = trunc(
		    cost * (1 - (owners[stores[store_num].owner]).max_inflate));
		min_buy = trunc(
		    cost * (1 - (owners[stores[store_num].owner]).min_inflate));
		if (min_buy < max_buy) {
			min_buy = max_buy;
		}
		...
		*price =
		...
		if (!(flag)) {
			...
			if (max_buy > max_gold) {
				...
				cur_ask = max_gold;
				final_ask = max_gold;
				msg_print("I am sorry, but I have not the "
					  "money to afford such a fine item.");
				msg_print(" ");
			} else {
				cur_ask = max_buy;
				final_ask = min_buy;
		     ...
		     boolean blitz)
{
	...
	*price = 0;
	...
	max_buy =
	    trunc(cost * (1 - owners[stores[store_num].owner].max_inflate));
	min_per = owners[stores[store_num].owner].haggle_per;
	max_per = min_per * 3.0;
