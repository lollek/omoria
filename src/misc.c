...
void sort_objects()
{
	...
	while (gap > 0) {
		for (i1 = gap + 1; i1 <= MAX_OBJECTS; i1++) {
			...
			while (i2 > 0) {
				i3 = i2 + gap;
				if (object_list[i2].level >
				    object_list[i3].level) {
					tmp = object_list[i2];
...
void init_t_level()
{
	...
	do {
		while ((i1 <= MAX_OBJECTS) && (object_list[i1].level == i2)) {
			t_level[i2] = t_level[i2] +
				      1; // number of treasures with this level
			i1++;
		}
		i2++;
...
void price_adjust()
{
	...
	for (i1 = 1; i1 <= INVEN_INIT_MAX; i1++) {
		inventory_init[i1].cost =
		    trunc(inventory_init[i1].cost * COST_ADJ + 0.99);
	}
}
...
void check_kickout_time(integer num, integer check)
{
	...
	if ((num % check) == 1) {
		if (check_kickout()) {
			find_flag = false;
			msg_print(
			    "A new version of IMORIA is being installed.");
...
void add_days(game_time_type *ti, integer d)
{
	...
	// 10/26/00 -- JEB:
	// DMF's code works great (if a little strangely) if you only ever add 1
	// day,
	// which this function did.  notice that the above code ignores the 'd'
	// parameter, which in turn means that no matter how long of a stay you
	// buy
	// in the inn, you really only get 1 day.  i thought about just putting
	// a loop
	// around the above code to iterate 'd' times, but that's lame so here's
	// some
	// more robust code that simply calculates the day, month, and year
	// increments
	// for any value of 'd'.  note that the above code implies that the year
	// is
	...
	mos =
	    (int)(d /
		  28); // mos = how many months you get from the remaining days
...
void validate_monsters()
{
	...
	for (i2 = 0, i1 = mfptr; i1; i2 = i1, i1 = m_list[i1].nptr) {
		if (free_list[i1]) {
			 ...
			 */
			report_mlist_error(
			    "Internal Error: m_list has a free list loop!", i1,
			    i2);
			busted = true;
			break;
		} else {
		...
		if (used_list[i1]) {
			 ...
			 */
			report_mlist_error(
			    "Internal Error: m_list lists overlap!", i1, i2);
			busted = true;
			break;
		}
	}
...
void place_object(integer y, integer x)
{
	...
	cave[y][x].tptr = cur_pos;
	t_list[cur_pos] =
	    object_list[get_obj_num(dun_level, PLACE_OBJECT_TRIES)];
...
boolean send_page(integer to_bank)
{
	...
	if (get_yes_no("Do you wish to send a page to the bank for money?")) {
		...
		if (py.misc.account < to_bank) {
			...
			sp__takey_munny(coin_value[MITHRIL], &bank[MITHRIL],
					&to_bank, &from_bank);
			sp__takey_munny(coin_value[PLATINUM], &bank[PLATINUM],
					&to_bank, &from_bank);
			sp__takey_munny(GOLD_VALUE, &bank[GOLD], &to_bank,
					&from_bank);
			if (randint(mugging_chance) == 1) {
				msg_print("The page was mugged while returning "
					  "from the bank!");
				sprintf(out_val,
					"You have lost %ld gold pieces!",
					from_bank);
				msg_print(out_val);
			} else {
				sprintf(
				    out_val,
				    "The page returns with %ld gold pieces.",
				    from_bank);
				msg_print(out_val);
				subtract_money(
		...
		msg_print(
		    "You cannot buy that with the money you are carrying.");
	}
...
char *full_date_string(game_time_type time, string result)
{
	...
	// with time do;
	sprintf(result, "%s, %s the %s, %s", out1,
...
void place_win_monster()
{
	...
	if (!total_winner) {
		...
		if (uand(c_list[m_list[cur_pos].mptr].cdefense, 0x4000) != 0) {
			...
			m_list[cur_pos].hp =
			    damroll(c_list[m_list[cur_pos].mptr].hd);
		}
