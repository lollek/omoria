	...
	do {
		...
		if (c_list[count].level > player_lev) {
			...
			do {
			    ...
			    (!((c_list[tmp_select].cmove & 0x00008000) == 0)) &&
			    tmp_select < MAX_CREATURES - 1);
		}
...
void q__explain_quests()
{
	...
	prt("I see that you have come, like many before you, in an effort to",
	    8, 10);
	prt("defeat the great evil that lies deep within the darkest bowels of",
	    9, 10);
	prt("the caves of Moria.  As you well know, none have yet succeeded.",
	    10, 10);
	prt("You, however . . . I feel that you are going to be different than",
	    12, 10);
	prt("the rest.  You have it within you to defeat the accursed Balrog,",
	    13, 10);
	prt("and restore peace and happiness to the people of this fair town.",
	    14, 10);
	prt("But you cannot do it alone.  You will need someone to guide you.",
	    15, 10);
	prt("I am willing to be that person, if you will have me.  I will aid",
	    17, 10);
	prt("you in your quest to defeat the Balrog.  But in return, you must",
	    18, 10);
	prt("complete many other, simpler quests.  These will strengthen you",
	    19, 10);
	prt("and prepare you for your final conflict, many moons hence.", 20,
	    10);
	...
	prt("I will give you a cash reward for the completion of each quest,",
	    6, 10);
	prt("to provide incentive for you.  In addition, I may occasionally", 7,
	    10);
...
void q__parse_command(boolean enter_flag)
{
	...
	do {
		if (get_com("", &command)) {
			...
			case 112: /* p */
				msg_print(
				    "Which quest would you like? [a-e]  ");
				break;
