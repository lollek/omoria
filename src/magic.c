...
static void mage_spell_effects(integer effect)
{
	...
	case 7: /*{ Stinking Cloud }*/
		if (d__get_dir("Which direction?", &dir, &dumy, &y_dumy,
			       &x_dumy)) {
			fire_ball(c_gas, dir, char_row, char_col, 9,
				  "Stinking Cloud");
		}
		break;
	...
	case 22: /*{ Phantasmal Force }*/
		if (d__get_dir("Which direction?", &dir, &dumy, &y_dumy,
			       &x_dumy)) {
			fire_bolt(c_illusion, dir, char_row, char_col,
				  damroll("8d4"), "phantasmal force");
		}
		break;
	...
	case 27: /*{ Hold Monster }*/
		if (d__get_dir("Which direction?", &dir, &dumy, &y_dumy,
			       &x_dumy)) {
			zap_monster(dir, char_row, char_col, 4 + randint(4),
				    c_hold);
		}
		break;
	...
	case 31: /*{ Frost Ball }*/
		if (d__get_dir("Which direction?", &dir, &dumy, &y_dumy,
			       &x_dumy)) {
			fire_ball(c_cold, dir, char_row, char_col, 40,
				  "Frost Ball");
		}
		break;
	...
	case 37: /*{ Fire Ball }*/
		if (d__get_dir("Which direction?", &dir, &dumy, &y_dumy,
			       &x_dumy)) {
			fire_ball(c_fire, dir, char_row, char_col, 59,
				  "Fire Ball");
		}
		break;
	...
	case 39: /*{ Power Word: Kill }*/
		if (d__get_dir("Which direction?", &dir, &dumy, &y_dumy,
			       &x_dumy)) {
			fire_bolt(0, dir, char_row, char_col, damroll("24d8"),
				  "Black Bolt");
		}
		break;
...
void cast()
{
	...
	redraw = false;
	if (!get_item(&item_ptr, "Use which spell-book?", &redraw, i2,
	...
	if (!cast_spell("Cast which spell?", item_ptr, &choice, &chance,
			&redraw)) {
		return;
	}
