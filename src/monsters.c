	...
	for (int i = 0; i < num; i++) {
		...
		for (;;) {
			...
			if (dun_level == 0) {
				...
				long i3 =
				    m_level[monster_i] - m_level[monster_i - 1];
				monster_i =
				    randint(i3) + m_level[monster_i - 1];
			} else {
				monster_i =
				    randint(m_level[dun_level]) + m_level[0];
			}

			boolean ok_monster_found;
			if (!water) {
				ok_monster_found =
				    (((c_list[monster_i].cmove & 0x00008000) ==
				      0) &&
				     (((c_list[monster_i].cmove & 0x00000010) ==
				       0) ||
				      ((c_list[monster_i].cmove & 0x00000040) ==
				       0) ||
				      ((c_list[monster_i].cmove & 0x00800000) !=
				       0)));
			} else {
				ok_monster_found =
				    (((c_list[monster_i].cmove & 0x00008000) ==
