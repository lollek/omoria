...
{
	/*{make gup[this] unavailable (for later selection), decrement
	 * Num_left}*/
	...
	if (Gup[this->y][this->x].pos <= Num_left) {
		...
		last =
		    S_list[Num_left]; /*{switch gup[this].pos with top elmt}*/
...
void r__plot_water(long y, long x, long font, long tdir)
{
	...
	for (i1 = 1; i1 <= num_dots; i1++) {
		if (in_bounds(dots[i1].y, dots[i1].x)) {
			...
			if ((cave[dots[i1].y][dots[i1].x].fval ==
			     dopen_floor.ftval) ||
			    (cave[dots[i1].y][dots[i1].x].fval ==
			     lopen_floor.ftval)) {
				cave[dots[i1].y][dots[i1].x].fval =
				    water2.ftval;
				cave[dots[i1].y][dots[i1].x].fopen =
				    water2.ftopen;
			} else {
				cave[dots[i1].y][dots[i1].x].fval =
				    water1.ftval;
			...
			if ((cave[dots[i1].y][dots[i1].x].tptr != 0) &&
			    (t_list[cave[dots[i1].y][dots[i1].x].tptr].tval >
			     valuable_metal)) {
...
long pr__figure_out_path_of_water(long y, long x, long oy, long ox)
{
	...
	for (i1 = 0; i1 <= 7; i1++) { /*{octant number}*/
		dot_product = target_dy * dy_of[key_of[i1]] +
			      target_dx * dx_of[key_of[i1]];
		...
		if (dot_product > 0) {
			/*if (i1 in [1,3,5,7]) {*/
			if (is_in(i1, odds)) {
				chance = dot_product * dot_product * 2 -
					 dist_squared;
			} else {
				chance = dot_product * dot_product * 4 -
					 dist_squared;
			}
		} else {
			chance = 0;
	...
	RETURN("pr__figure_out_path_of_water", "r", 'd', "key_of[i1]",
	       &return_value);
	return return_value;
}
/*//////////////////////////////////////////////////////////////////// */
		    ...
		    coords wiggle)
{
	...
	if (dir != next_dir) {
		i1 = oct_of[next_dir] -
		     oct_of[dir]; /*{ (1=left, -1 = right) mod 8}*/
		if ((oct_of[dir] % 2) == 0) {
...
void r__chart_river(river_args *a)
{
	...
	for (; cr__dequeue_s_list(a);) { /*{loop until river stops}*/
		if (starting_river) {
			...
			branches =
			    cr__choose_stream_dirs(a, &this, dir, that_dir,
						   that_ok, that, that_chosen);
		}
		...
		for (i1 = 1; i1 <= 3; i1++) {
			if (that_chosen[i1] &&
			    (RIVER_TOTAL_SIZE - Num_left < Max_wet)) {
				if (branches == 1) {
					in_flow = out_flow;
				} else {
				...
				if (in_flow > 0) {
					 ...
					 */
					if (i2 == 1) {
						Gup[this.y][this.x].in1 =
						    that_dir[i1];
					} else {
						Gup[this.y][this.x].in2 =
						    that_dir[i1];
					}
					S_list[Gup[that[i1].y][that[i1].x].pos]
					    .is_active = true;
					Gup[that[i1].y][that[i1].x].out =
					    that_dir[i1];
					Gup[that[i1].y][that[i1].x].flow =
					    in_flow;
				}
				i2++;
			}
...
void r__draw_river(river_args *a)
{
	...
	if (Gup[that.y][that.x].in1 != 5) {
		r__place_river(a, first_dir, Gup[that.y][that.x].in1,
			       River_mouth, wiggle);
	}
	if (Gup[that.y][that.x].in2 != 5) {
		r__place_river(a, first_dir, Gup[that.y][that.x].in2,
			       River_mouth, wiggle);
	}
...
void all_the_river_stuff()
{
	...
	for (i1 = 1; i1 <= a.num_left; i1++) {
		/* with s_list[i1]. do; */
		if ((a.s_list[i1].loc.y == 1) ||
		    (a.s_list[i1].loc.y == RIVER_SIZE_Y) ||
		    (a.s_list[i1].loc.x == 1) ||
		    (a.s_list[i1].loc.x == RIVER_SIZE_X)) {
