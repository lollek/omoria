...
## Unreleased
* Internal: torch lighting now uses a `LIGHT_RADIUS` constant (starting at 1) and Rust-backed LOS checks to prevent light bleeding through walls at larger radii.
...
* Internal: `Sex` now uses model-layer Rust `From`/`TryFrom` conversions for C character codes, with conversion wrappers kept as deprecated compatibility shims.
...
* Internal: Item subtype migration: `FlaskOfOilSubType` now uses model-layer `From`/`TryFrom` conversions, with compatibility wrappers still available.
* Internal: Item subtype migration: `LodgingAtInnSubType` now uses model-layer `From`/`TryFrom` conversions, with compatibility wrappers still available.
* Internal: Item subtype migration: `SongBookSubType` now uses model-layer `From`/`TryFrom` conversions, with compatibility wrappers still available.
